...
public class YCBAQueryTagsHandler implements YCBARequestHandler.Callback {
    ...
    public final HashMap<String, Tag> mNameToTagHashMap = new HashMap<>();
    ...
    private void updateFetchedCount() {
        ...
        if (mFetchedCount == mCurrentPiecesCount) {
            ...
            for (String tagName : mNameToTagHashMap.keySet()) {
                Log.d("YCBAQueryTagsHandler", tagName + " " + mNameToTagHashMap.get(tagName).mIdsWithThisTag);
                firebaseRequestHandler.setTagAndListOfIds(mNameToTagHashMap.get(tagName));
            }
        }
    }
