...
public class YCBAQueryTagsHandler implements YCBARequestHandler.Callback {
    ...
    public final HashMap<Tag, Integer> mTagToFrequencyHashMap = new HashMap<>();
    ...
    private void updateTagCount(Artwork artwork) {
        ...
        if (tags != null && tags.size() > 0) {
            for (Tag tag : tags) {
                Integer count = mTagToFrequencyHashMap.get(tag);
                count = (count == null) ? 1 : count + 1;
                Tag newTag = new Tag.Builder().fromOld(tag).setCount(count).build();
                mTagToFrequencyHashMap.put(newTag, count);
            }
        }
    }
    ...
    private void updateFetchedCount() {
        ...
        if (mFetchedCount == mCurrentPiecesCount) {
            ...
            for (Tag tag : tags) {
                Log.d("YCBAQueryTagsHandler", tag + " " + mTagToFrequencyHashMap.get(tag.mName));
            }
        }
    }
