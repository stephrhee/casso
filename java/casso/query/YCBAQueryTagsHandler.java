...
public class YCBAQueryTagsHandler implements YCBARequestHandler.Callback {
    ...
    public final HashMap<String, List<Integer>> mTagToListOfIdsHashMap = new HashMap<>();
    ...
    private void updateHashMap(Artwork artwork) {
        List<Tag> tags = artwork.mTags;
        if (tags != null && tags.size() > 0) {
            for (Tag tag : tags) {
                List<Integer> listOfIds = mTagToListOfIdsHashMap.get(tag.mName);
                if (listOfIds == null) {
                    listOfIds = new ArrayList<>();
                }
                listOfIds.add(artwork.mId);
                Tag newTag = new Tag.Builder()
                        .fromOld(tag)
                        .setCount(listOfIds.size())
                        .build();
                mTagToListOfIdsHashMap.put(newTag.mName, listOfIds);
            }
        }
    }
    ...
    private void updateFetchedCount() {
        ...
        if (mFetchedCount == mCurrentPiecesCount) {
            ...
            tags.addAll(mTagToListOfIdsHashMap.keySet());
            for (String tag : tags) {
                Log.d("YCBAQueryTagsHandler", tag + " " + mTagToListOfIdsHashMap.get(tag));
            }
        }
    }
